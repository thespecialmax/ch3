...
(function($){
  ...
  theme.ProductScreen = (function() {
    ...
    ProductScreen.prototype.close = function(evt, args) {
      ...
      this.$screen
        ...
        .addClass(closeClass);
      this.nodes.$parent.removeClass(this.config.bodyOpenClass);
      this.nodes.$parent.addClass(bodyCloseClass);
  ...
  theme.AjaxCart = (function() {
    ...
    AjaxCart.prototype = $.extend({}, AjaxCart.prototype, {
      ...
      initEventListeners: function() {
        ...
        if (this.status.isDrawer) {
          $('body')
            .on('updateCart' + config.namespace, this.sizeCart.bind(this))
            .on('drawerOpen.' + this.id, function() {
              this.sizeCart();
              setTimeout(function() {
                this.sizeCart();
              }.bind(this), 500);
          ...
          $(window).on('resize' + config.namespace, $.debounce(150, this.sizeCart.bind(this)));
        }
      },
      ...
      buildCart: function(cart, openDrawer) {
        ...
        if (window.Shopify && Shopify.StorefrontExpressButtons) {
          ...
          // Resize footer after arbitrary delay to account for checkout buttons
          if (this.status.isDrawer) {
            setTimeout(function() {
              this.sizeCart();
            }.bind(this), 1000);
      ...
      _createCart: function(cart) {
        ...
        $(selectors.cartDiscountContainer, $form)
          .toggleClass('hide', cart.total_discount === 0);
      ...
      _onItemDelete: function(evt) {
        ...
        if ($items.length === 2 && $items.last().is($note)) {
          ...
          slate.a11y.promiseTransitionEnd($(selectors.itemList, this.$container)).then(function() {
        ...
        slate.a11y.promiseAnimationEnd($item).then(function() {
      ...
      sizeCart: function() {
        // If drawer cart is set to static, measure to make sure it should
        // stay that way
        if (this.$container.hasClass(classes.staticDrawerElement)) {
          // Add static class, measure, and decide if we need to add it back
          this.$container.addClass(classes.staticDrawerElement);
  
          // If cart is taller than window, remove static class so
          // elements become absolute and items scrollable
          if (this.$container.height() >= window.innerHeight) {
            this.$container.removeClass(classes.staticDrawerElement);
          }
        }
  
        // Stop if our drawer doesn't have a fixed footer
        if (!this.$container.hasClass('drawer--has-fixed-footer')) {
          return;
        }
  
        // Elements are reprinted regularly so selectors are not cached
        var $cartFooter = this.$container.find(selectors.fixedFooter).removeAttr('style');
        var $cartInner = this.$container.find(selectors.fixedInnerContent).removeAttr('style');
  ...
  theme.currencySwitcher = (function() {
    ...
    var selectors = {
      ...
      pickerFlag: '#CurrencyFlag',
      pickerLabel: '#CurrencyLabel'
    };
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      formSetup: function() {
        ...
        if (theme.settings.dynamicVariantsEnable) {
          this.$variantSelectors = $(selectors.formContainer).find(selectors.singleOptionSelector);
        }
      ...
      updateInventory: function(evt) {
        ...
        var quantity = 0;
        var $inventoryLabel = $(selectors.inventory, this.$container);
  
        if (!window.inventories || !window.inventories[this.sectionId]) {
          // Force reload if in editor
          if (Shopify.designMode) {
            location.reload();
          }
          return;
      ...
      setCurrentVariantAvailability: function(variant) {
        ...
        this.disableVariantGroup($(selectors.formContainer).find('.variant-input-wrap'));
      ...
      updateVariantAvailability: function(evt, value, index) {
        ...
        $(selectors.formContainer).find('.variant-input-wrap').each(function(index, el) {
          var $group = $(el);
      ...
      enableVariantOptionByValue: function(array, index) {
        var $group = $(selectors.formContainer).find('.variant-input-wrap[data-index="'+ index +'"]');
  ...
  theme.FeaturedCollection = (function() {
    ...
    function FeaturedCollection(container) {
      ...
      this.options = {
        ...
        instagram: this.$container.data('instagram')
      };
    ...
    FeaturedCollection.prototype = $.extend({}, FeaturedCollection.prototype, {
      checkVisibility: function() {
        ...
        var visibleThreshold = this.options.instagram ? 500 : 0;
  
        if (theme.isElementVisible(this.$container, visibleThreshold)) {
      ...
      initInstagram: function() {
        $('.instagram-image', this.$container).each(function(i, el) {
          if ($(el).is('[data-url]')){
            this.getInstagramImage($(el));
          }
        }.bind(this));
      },
  
      getInstagramImage: function($el) {
        var instagramLink = $el.attr('data-url')
        ...
        if ( regex.test(instagramLink) ){
          ...
          var oembed_url = 'https://www.instagram.com/p/' + encodeURIComponent( shortcode ) + '/media/?size=l';
          var request = new XMLHttpRequest();
          request.open('GET', oembed_url, true);
          request.onload = function() {
            if (request.status >= 200 && request.status < 400) {
              $el.css('background-image', 'url(' + request.responseURL + ')' );
            } else {
              console.warn('Instagram request failed');
            }
          };
          ...
          $el
            .html('Invalid Instagram link. Use this format:<small>https://instagram.com/p/BeQb_V9jyDr</small>')
            .addClass('instagram-image--error');
        }
      },
  ...
  theme.Collection = (function() {
    ...
    function Collection(container) {
      ...
      var hasHeroImage = $(container).find('.collection-hero').length;
  ...
  theme.pageTransitions = function() {
    if ($('body').data('transitions') == true) {
      ...
      $('a:not(.js-no-transition)').bind('click', function(evt) {
        ...
        // Bail if it's a hash link
        if(src.indexOf(location.pathname) >= 0) {
          return true;
        }
...
function toggleShiftMenu(){
  $('body').toggleClass('shifter-active');
}

function openChild(e,id){
  e.preventDefault();
  e.stopPropagation();
  $("#"+id).slideToggle();
}

function closeStickyCart(){
  $('#StickyCart').removeClass('sticky-cart--open');
}